...
$.widget( "mobile.panel", $.mobile.widget, {
	...
	_create: function() {
		...
		// ######################################################################
		// Hello, I am a hack so panels work in the builder.
		// This keeps track of which eventNamespace is the most recent for this element
		// There's also a hack in _bindLinkListeners().... :derp:
		window.CodiqaPanels = window.CodiqaPanels || {};
		window.CodiqaPanels[ this.element.attr('id') ] = this.eventNamespace;
		// ######################################################################
	...
	_bindLinkListeners: function() {
		...
		self._page.on( "click.panel" , "a", function( e ) {
			...
			// ######################################################################
			// Hello, I am a hack so panels work in the builder.
			// There's also a hack in _create().... :derp:
			if( window.CodiqaPanels && self.eventNamespace !== window.CodiqaPanels[ self.element.attr('id') ] ) {
				// if its not the most recent eventNamespace for this element
				// then don't do anything cuz the old element is long gone and no worky
				return;
			}
			// ######################################################################
